TMDGen softawre created by Nuruzzaman Sakib, University of Alabama, USA
...
1. Change the bash file "TMDGen.sh" according to the Grain Boundary misorientaion angle. Just change the "r1","r2", "cell_width".
2. run bash TMDGen.sh from outside of Codes directory. Do not change the folder and TMDGen location. If you do, then change the bash script as well to accomodate the change.
3. After that, you need to run energy minimization of the created TMDC GB structure before use in LAMMPS calculation.
